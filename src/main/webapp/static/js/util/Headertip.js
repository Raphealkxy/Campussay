	...
	function wmHeadertipWrap(message,closeable,timeout,document){
		...
		if(closeable){
			...
			if($('#wmHeadertip_alert button',document).length <= 0){
				$('#wmHeadertip_alert',document).append('<button type="button" class="close" data-dismiss="alert" aria-hidden="true" style="background-color: #00caab;color: #fff">&times;</button>');
			}

			$('#wmHeadertip_alert button',document).click(destroy);
		}
		$('#wmHeadertip_alert',document).css('text-align','center');
		$('#wmHeadertip_alert',document).append('<span id="wmHeadertip_message" style="text-align:center;background-color: #00caab;color: #fff;padding-right: 5px;">'+message+'</span>');
