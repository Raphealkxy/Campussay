(function(factory) {
    ...
    if (typeof define === 'function' && define.amd) {
        define(['lib/jquery', 'modules/textEditor/wysiwyg', 'modules/fileUpload/ajaxfileupload'], function($, wysiwyg, fileUpload) {
            return factory(window, document, $, wysiwyg);
        });
    } else if (typeof exports !== 'undefined') {
        module.exports = factory(window, document, require('lib/jquery'));
    } else {
        return factory(window, document, jQuery);
    }
})(function(window, document, $, wysiwyg) {
    'use strict';
    ...
    var create_editor = function($textarea, classes, placeholder, toolbar_position, toolbar_buttons, toolbar_submit, label_selectImage,
        ...
        var content_insertimage = function(wysiwygeditor) {
            // Add image to editor
            var insert_image_wysiwyg = function(url) {
                var html = '<img id="wysiwyg-insert-image" src="" >';
            ...
            var $fileuploader = null,
                $fileuploader_input = $('<input type="file" id="J_loadImg" name="J_loadImg" />')
                .css({
                    position: 'absolute',
                    left: 0,
                    top: 0,
            ...
            if (!force_imageupload && window.File && window.FileReader && window.FileList) {
                ...
                /* var loadImageFromFile = function( file )
                 {
                     ...
                     reader.onload = function(event) {
                         var dataurl = event.target.result;//base64
                ...
                var loadImageFromFile = function(url) {
                    if (url) {
                        var tmpl = '<div><img src=' + url + 'sytle="width:100px;height:100px"  /></div>';
                    }
                    insert_image_wysiwyg( url );
                }
                    ...
                    .change(function(event) {
                        ...
                        for (var i = 0; i < files.length; ++i) {
                            $.ajaxFileUpload({
                                ...
                                secureuri: false,
                                fileElementId: "J_loadImg", //文件选择框的id属性
                                dataType: 'json',
                                success: function(res) {
                                    var url = res.data.url;
                                    if(url){
                                        loadImageFromFile(window.pub.imgServer+url);
                                    }
                                },
                ...
                // Upload image to a server ：fileuploader_input
